...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\linewidth]{figures/transfer_distance.png}
  \caption{Semantic-type embedding space. Each region declares its modality as a string plus a frozen embedding from a fixed model; cosine distance between embeddings estimates the cost of bridging two modalities (adapter layers, data). Camera$\leftrightarrow$depth is cheap; camera$\leftrightarrow$joint-angles is expensive. This turns modality compatibility into a computable quantity---contingent on the representation-stability hypothesis (\S\ref{sec:open}).}
  \label{fig:transfer}
\end{figure}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.46\textwidth]{figures/04_fleet.png}\hfill
  \includegraphics[width=0.46\textwidth]{figures/06_atc.png}
  \caption{Two multi-agent canvases. \textbf{Left:} cooperative vehicle-fleet trajectory prediction, per-vehicle regions coordinating through a shared dispatch hub via coarse-grained summaries. \textbf{Right:} air-traffic conflict routing with 12 aircraft. In both, the attention topology is the coordination protocol expressed as data rather than a pattern the network must rediscover (both are architectural demonstrations, not deployed systems).}
  \label{fig:fleet}
\end{figure*}
